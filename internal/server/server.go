...
func New(addr, hostKey string, l *lobby.Lobby) (*ssh.Server, error) {
	return wish.NewServer(
		...
		wish.WithMiddleware(
			bubbletea.Middleware(handler(l)),
...
func handler(l *lobby.Lobby) bubbletea.Handler {
	return func(s ssh.Session) (tea.Model, []tea.ProgramOption) {
		model := tui.NewWithRenderer(l, playerFor(s), bubbletea.MakeRenderer(s))
